  ...
  return (
    <div className="rounded-2xl border border-slate-200 p-4 sm:p-5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        {(Object.keys(value.fees) as FeeKey[]).map((key) => <FeeField key={key} feeKey={key} label={key === "serviceFee" ? "Service Fee" : key === "processingFee" ? "Processing Fee" : key === "lateFee" ? "Default / Late Fee" : "Interest"} baseFee={value.fees[key]} value={value.fees[key]} onChange={(fee) => patch({ fees: { ...value.fees, [key]: fee } })} />)}
      </div>
