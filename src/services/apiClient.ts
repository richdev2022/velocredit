...
export async function register(input: RegisterInput): Promise<AuthResponse> {
  const response = await request<AuthResponse>("/api/v1/auth/register", { method: "POST", body: JSON.stringify(input) });
...
export interface OtpRequestResponse { ok: true; challengeId: string; expiresAt: string; delivered: boolean; channel?: NotificationChannel; }
...
export interface KycUpdateInput { status?: KycStatus; checklistOverride?: Partial<KycChecklist>; bvn?: string; nin?: string; }
...
export async function submitKyc(): Promise<KycResponse> {
  return updateMyKyc({ status: "PENDING_VERIFICATION" });
}
...
export interface BorrowerDashboardResponse { ok: true; applications: unknown[]; loans: unknown[]; repayments: unknown[]; }
