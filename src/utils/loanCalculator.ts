...
export function calculateDailyInterest(amount: number, fee: FeeConfiguration["interest"]): number {
...
export function calculateTermInterest(amount: number, fee: FeeConfiguration["interest"], tenureDays: number): number {
  const dailyInterest = calculateDailyInterest(amount, fee);
  return fee.type === "percentage" ? Math.round(dailyInterest * tenureDays) : dailyInterest;
}
