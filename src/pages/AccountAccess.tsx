...
export default function AccountAccess() {
  ...
  const { login, register, requestPasswordReset, confirmPasswordReset, user } = useAuth();
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      if (mode === "login") {
        ...
        const newUser = await register({ email, phone, fullName, password, role });
        if (role === "BORROWER") {
          navigate(`/apply?type=${loanType}`, { replace: true });
        } else {
              ...
              <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-xl shadow-slate-200/50 dark:shadow-slate-950/50 border border-slate-100 dark:border-slate-800 p-6 sm:p-7 overflow-hidden relative">
                ...
                <div className="relative">
                  ...
                  <form onSubmit={submit} className="space-y-4">
                    ...
                    {(mode === "login" || mode === "register" || mode === "forgot") && !(mode === "forgot" && resetId && resetToken) && (
                      <label className="block">
                    ...
                    <button
                      ...
                      disabled={busy || (mode === "register" && (!passwordMatch || !passwordStrong))}
                    >
                      {busy ? (
                        ...
                        role === "INVESTOR" ? "Create investor account" : "Create borrower account"
                      )}
                    </button>
