...
const COLUMNS = Object.freeze([
  ...
  'Employment Status', 'Employer/Business Name',
  'Monthly Income', 'Monthly Expenses',
  'Business Revenue', 'Business Expenses',
  'Existing Loan Obligations', 'Expected Repayment Source',

  'Loan Amount', 'Loan Tenure', 'Loan Purpose',
  'Collateral Type', 'Collateral Description', 'Collateral Value', 'Collateral Ownership', 'Collateral Location', 'Collateral Reference',
  'Interest', 'Service Fee', 'Processing Fee', 'Other Fees',
  'Total Fees', 'Total Repayment',
  'Disbursement Date', 'Repayment Date',

  'Google Drive Folder URL',
  'ID Document URL', 'Proof of Address URL', 'Collateral Media URL', 'Signed Agreement URL',
...
function handleSaveDraft(payload) {
  return withScriptLock_(function() {
    ...
    const status = app.status === 'DRAFT' || app.status === 'IN_PROGRESS'
      ? app.status
      : 'IN_PROGRESS';
    ...
    const applicantFolder = (applicantEmail || hasDocuments)
      ? getOrCreateApplicantFolder_(parentFolder, app.applicationId, applicantName, applicantEmail)
      : null;
    const docUrls = applicantFolder ? uploadDocuments_(applicantFolder, app) : {};

    const rowData = buildRowData_(app, status, {
      lastSectionIndex: payload && payload.lastSectionIndex != null ? payload.lastSectionIndex : (app.lastSectionIndex != null ? app.lastSectionIndex : ''),
...
function handleSubmit(payload) {
  return withScriptLock_(function() {
    ...
    if (existingOriginalRow !== -1) {
      ...
      if (['SUBMITTED', 'UNDER_REVIEW', 'APPROVED', 'REJECTED', 'DISBURSED', 'REPAID'].indexOf(existingStatus) !== -1) {
        return {
          ok: true,
    ...
    const applicantName = getApplicantDisplayName_(app);
    const applicantEmail = getApplicantEmail_(app);
    const applicantFolder = getOrCreateApplicantFolder_(parentFolder, finalId, applicantName, applicantEmail);
...
function handleLookup(payload) {
  ...
  for (let i = data.length - 1; i >= 1; i--) {
    ...
    if (['SUBMITTED', 'UNDER_REVIEW', 'APPROVED', 'REJECTED', 'DISBURSED', 'REPAID'].indexOf(status) !== -1) {
      return {
        ok: true,
        found: true,
        ...
        applicationId: id,
        status: status,
        message: 'An application with these details has already been submitted. Reference: ' + id
      };
    }

    const applicantTypeRaw = String(valueAt_(row, indexes.type) || 'PERSONAL').toUpperCase();
    const applicantType = applicantTypeRaw === 'BUSINESS' ? 'BUSINESS' : 'PERSONAL';

    const loanAmount = numberOrZero_(valueAt_(row, indexes.loanAmount));
    const loanTenureRaw = valueAt_(row, indexes.loanTenure);
    const loanTenure = loanTenureRaw === '' || loanTenureRaw == null ? 30 : Number(loanTenureRaw) || 30;
    const lastSectionRaw = valueAt_(row, indexes.lastSectionIndex);
    const lastSectionIndex = lastSectionRaw === '' || lastSectionRaw == null ? null : (Number(lastSectionRaw) || 0);
    ...
    const application = {
      ...
      personalFinancial: {
        ...
        existingLoanObligations: String(valueAt_(row, indexes.existingLoans) || ''),
        expectedRepaymentSource: String(valueAt_(row, indexes.repaymentSource) || '')
      },

      businessInfo: {
        businessName: String(valueAt_(row, indexes.businessName) || ''),
        businessRegistrationNumber: String(valueAt_(row, indexes.businessRegNo) || ''),
      ...
      businessFinancial: {
        averageMonthlyRevenue: String(valueAt_(row, indexes.businessRevenue) || ''),
        averageMonthlyExpenses: String(valueAt_(row, indexes.businessExpenses) || ''),
        existingLoanObligations: String(valueAt_(row, indexes.existingLoans) || ''),
        expectedRepaymentSource: String(valueAt_(row, indexes.repaymentSource) || '')
      },
      ...
      collateral: {
        ...
        documentReference: String(valueAt_(row, indexes.collateralReference) || '')
      },
      calculation: {
      ...
      documents: valueAt_(row, indexes.collateralMediaUrl) ? { collateralMedia: { slot: 'collateralMedia', name: 'Collateral evidence', type: '', size: 0, data: '', status: 'uploaded', driveUrl: String(valueAt_(row, indexes.collateralMediaUrl)) } } : {},
      agreement: {
        generatedAt: null,
        generatedHtml: null,
...
function getSpreadsheet_() {
  ...
  for (let i = 0; i < candidates.length; i++) {
    try {
      ...
      console.warn('Unable to open configured spreadsheet ID: ' + candidates[i]);
    }
  }
...
function ensureHeaderRow_(sheet) {
  ...
  const hasAnyHeader = firstRow.some(function(v) { return String(v || '').trim() !== ''; });
  ...
  const existingHeaders = firstRow.map(function(v) { return String(v || '').trim(); });
...
function formatSheet_(sheet) {
  ...
  sheet.getRange(1, 1, Math.max(sheet.getLastRow(), 1), lastCol).setVerticalAlignment('middle');
...
function getDriveRootFolder_() {
  ...
  for (let i = 0; i < candidates.length; i++) {
    try {
      ...
      console.warn('Unable to open configured Drive folder ID: ' + candidates[i]);
    }
  }
...
function buildRowData_(app, status, opts) {
  ...
  row['Date Submitted'] = status === 'SUBMITTED' ? (app.submittedAt || nowIso) : (app.submittedAt || '');

  row['Full Name'] = personalInfo.fullName || '';
  row['Date of Birth'] = firstNonEmpty_(personalInfo.dateOfBirth, businessRep.dateOfBirth);
  ...
  row['Business Registration Number'] = businessInfo.businessRegistrationNumber || '';
  ...
  row['Business Revenue'] = numberOrBlank_(businessFinancial.averageMonthlyRevenue);
  row['Business Expenses'] = numberOrBlank_(businessFinancial.averageMonthlyExpenses);
  ...
  row['Loan Amount'] = numberOrZero_(calc.loanAmount != null ? calc.loanAmount : loanRequest.amount);
  ...
  row['Other Fees'] = numberOrZero_(calc.otherFees != null ? calc.otherFees : calc.lateFee);
  ...
  row['Last Section Index'] = opts.lastSectionIndex != null ? opts.lastSectionIndex : '';
...
function uploadDocuments_(folder, app) {
  ...
  slots.forEach(function(slot) {
    ...
    if (CONFIG.ALLOWED_MIME.indexOf(mime) === -1) {
      throw new Error('Disallowed file type for ' + slot.key + ': ' + (doc.type || 'unknown'));
    }
    ...
    if (bytes.length > CONFIG.MAX_FILE_SIZE_BYTES) {
      throw new Error(
        'File too large for ' + slot.key + ': ' +
        (bytes.length / 1024 / 1024).toFixed(1) + ' MB. Maximum is 10 MB.'
      );
    }
    ...
    file.setDescription('Velo Finance loan application ' + app.applicationId + ' — ' + slot.key);
...
function getOrCreateApplicantFolder_(parentFolder, applicationId, applicantName, applicantEmail) {
  ...
  const folderName = sanitizeDriveName_(safeApplicantName + (safeApplicantEmail ? ' - ' + safeApplicantEmail : ' - ' + applicationId));
...
function getLoanRules_(applicantType) {
  ...
  const program = overrides.loanPrograms && overrides.loanPrograms[String(applicantType || 'PERSONAL').toUpperCase()];
  const source = program || overrides;
  const limits = Object.assign({ min: 100000, max: 30000000, defaultAmount: 3000000 }, source.loanLimits || {});
  const tenures = (source.tenures || [30, 60, 90, 180].map(function(value) { return { value: value, label: value + ' Days' }; }))
    .map(function(item) { return Number(item.value); }).filter(function(value) { return Number.isFinite(value) && value > 0; });
  const defaults = {
  ...
  return { limits: limits, tenures: tenures, fees: fees, tenureFees: source.tenureFees || {} };
}

function backendFee_(amount, fee) {
  if (!fee) return 0;
  return Math.round(fee.type === 'percentage' ? amount * Number(fee.value || 0) / 100 : Number(fee.value || 0));
}
...
function calculateBackendLoan_(amount, tenure, now, applicantType) {
  ...
  const interest = fees.interest && fees.interest.type === 'percentage' ? Math.round(backendFee_(safeAmount, fees.interest) * safeTenure) : backendFee_(safeAmount, fees.interest);
  ...
  const totalFees = (fees.interest.includeUpfront ? interest : 0) + (fees.serviceFee.includeUpfront ? serviceFee : 0) + (fees.processingFee.includeUpfront ? processingFee : 0) + (fees.lateFee.includeUpfront ? lateFee : 0);
  ...
  return { loanAmount: safeAmount, interest: interest, serviceFee: serviceFee, processingFee: processingFee, lateFee: lateFee, totalFees: totalFees, totalRepayment: safeAmount + totalFees, upfrontFees: totalFees, loanCost: interest, defaultFee: lateFee, tenure: safeTenure, tenureLabel: safeTenure + ' Days', disbursementDate: disbursement.toISOString(), repaymentDate: repayment.toISOString() };
}
...
function validateLoanTerms_(app, requirePurpose) {
  ...
  if (!Number.isFinite(amount) || amount < Number(rules.limits.min) || amount > Number(rules.limits.max)) throw new Error('Loan amount must be between ' + Number(rules.limits.min).toLocaleString() + ' and ' + Number(rules.limits.max).toLocaleString() + '.');
  if (!rules.tenures.some(function(value) { return value === tenure; })) throw new Error('Selected loan tenure is not available.');
  if (requirePurpose && String(request.purpose || '').trim().length < 10) throw new Error('Loan purpose must be at least 10 characters.');
  const adminConfig = getAdminConfig_();
  const program = adminConfig.loanPrograms && adminConfig.loanPrograms[String(app.applicantType || 'PERSONAL').toUpperCase()];
  const collateralRules = program && program.collateral;
  const collateral = app.collateral || {};
  if (collateralRules && collateralRules.enabled && collateralRules.required) {
    if (!collateral.type || !collateral.description || !collateral.estimatedValue || !collateral.ownership || !collateral.location || !app.documents || !app.documents.collateralMedia || !app.documents.collateralMedia.data) throw new Error('Collateral details and supporting media are required for this loan type.');
  }
  app.loanRequest.amount = Math.round(amount);
  app.loanRequest.tenure = tenure;
  app.calculation = calculateBackendLoan_(amount, tenure, new Date(), app.applicantType);
}
...
function assertDraftOwnership_(sheet, rowIdx, app) {
  ...
  const existingStatus = String(existing['Application Status'] || '').toUpperCase();
  if (['SUBMITTED', 'UNDER_REVIEW', 'APPROVED', 'REJECTED', 'DISBURSED', 'REPAID'].indexOf(existingStatus) !== -1) throw new Error('This application can no longer be changed.');
  ...
  const storedEmail = normalizeEmail_(existing['Email'] || existing['Representative Email']);
  const storedPhone = normalizePhone_(existing['Phone'] || existing['Representative Phone']);
  const storedDob = String(existing['Date of Birth'] || '').slice(0, 10);
  const incomingDob = String(personal.dateOfBirth || representative.dateOfBirth || '').slice(0, 10);
  if ((storedEmail && incomingEmail !== storedEmail) || (storedPhone && incomingPhone !== storedPhone) || (storedDob && incomingDob && incomingDob !== storedDob)) throw new Error('Application ownership could not be verified.');
}
...
function validateForSubmission_(app) {
  ...
  requireDocument_(missing, app.documents, 'identificationDocument', 'identificationDocument');
  requireDocument_(missing, app.documents, 'proofOfAddress', 'proofOfAddress');
  requireDocument_(missing, app.documents, 'signedAgreement', 'signedAgreement');
...
function normalizeApplication_(app) {
  if (app.applicantType) app.applicantType = String(app.applicantType).trim().toUpperCase();
  if (app.status) app.status = String(app.status).trim().toUpperCase();

  if (app.personalInfo) {
    if (app.personalInfo.email) app.personalInfo.email = normalizeEmail_(app.personalInfo.email);
    if (app.personalInfo.phone) app.personalInfo.phone = String(app.personalInfo.phone).trim();
  }

  if (app.businessRep) {
    if (app.businessRep.email) app.businessRep.email = normalizeEmail_(app.businessRep.email);
    if (app.businessRep.phone) app.businessRep.phone = String(app.businessRep.phone).trim();
  }
...
function setAdminEmail(email) {
  ...
  if (!/^[^\\s@]+@[^\\s@]+\\.[^\\s@]+$/.test(value)) throw new Error('A valid administrator email is required.');
  PropertiesService.getScriptProperties().setProperty('VELO_ADMIN_EMAIL', value);
  return true;
}

function getAdminPassword_() {
  return PropertiesService.getScriptProperties().getProperty(PROP_KEYS.ADMIN_PASSWORD) || CONFIG.ADMIN_PASSWORD;
}
...
function handleAdminLogin(payload) {
  ...
  if (!expected) return { ok: false, error: 'Admin password has not been configured.' };
...
function isValidAdminToken_(token) {
  ...
  try {
    ...
    if (!data.issuedAt || Date.now() - Number(data.issuedAt) > CONFIG.ADMIN_TOKEN_TTL_MS) {
...
function handleAdminSaveConfig(payload) {
  ...
  ['adminEmails', 'loanManagerEmails', 'loanManagers'].forEach(function(key) {
    if (config[key] === undefined && previous[key] !== undefined) config[key] = previous[key];
  });
  PropertiesService.getScriptProperties().setProperty(PROP_KEYS.ADMIN_CONFIG, JSON.stringify(config));
...
function handleAdminResetConfig() {
  ...
  if (Object.keys(preserved).length) PropertiesService.getScriptProperties().setProperty(PROP_KEYS.ADMIN_CONFIG, JSON.stringify(preserved));
  else PropertiesService.getScriptProperties().deleteProperty(PROP_KEYS.ADMIN_CONFIG);
  return { ok: true };
}

function getAdminConfig_() {
  const raw = PropertiesService.getScriptProperties().getProperty(PROP_KEYS.ADMIN_CONFIG);
  if (!raw) return {};
  try {
...
function normalizeAdminConfig_(input) {
  ...
  if (limits && typeof limits === 'object' && !Array.isArray(limits)) {
    ...
    ['min', 'max', 'defaultAmount'].forEach(function(key) {
      ...
      if (Number.isFinite(value) && value >= 0 && value <= 1000000000) loanLimits[key] = Math.round(value);
    });
    if (Object.keys(loanLimits).length) result.loanLimits = loanLimits;
  }
  if (Array.isArray(input.tenures)) {
    const tenures = input.tenures.map(function(item) {
  ...
  function normalizeFee(fee) {
    ...
    const type = fee.type === 'flat' || fee.type === 'percentage' ? fee.type : null;
    const value = Number(fee.value);
    if (!type || !Number.isFinite(value) || value < 0 || value > (type === 'percentage' ? 100 : 1000000000)) return null;
    return { type: type, value: value, includeUpfront: fee.includeUpfront === true };
  }
  if (input.fees && typeof input.fees === 'object' && !Array.isArray(input.fees)) {
    const fees = {};
    feeKeys.forEach(function(key) { const fee = normalizeFee(input.fees[key]); if (fee) fees[key] = fee; });
    if (Object.keys(fees).length) result.fees = fees;
  }
  if (input.tenureFees && typeof input.tenureFees === 'object' && !Array.isArray(input.tenureFees)) {
    const tenureFees = {};
    Object.keys(input.tenureFees).forEach(function(days) {
      const tenure = Number(days);
      const candidate = input.tenureFees[days];
      if (!Number.isFinite(tenure) || tenure <= 0 || tenure > 3650 || !candidate || typeof candidate !== 'object') return;
      const fees = {};
      feeKeys.forEach(function(key) { const fee = normalizeFee(candidate[key]); if (fee) fees[key] = fee; });
  ...
  if (input.loanPrograms && typeof input.loanPrograms === 'object' && !Array.isArray(input.loanPrograms)) {
    const programs = {};
    ['PERSONAL', 'BUSINESS'].forEach(function(type) {
      ...
      if (candidate.loanLimits && typeof candidate.loanLimits === 'object') {
        const limits = {};
        ['min', 'max', 'defaultAmount'].forEach(function(key) { const value = Number(candidate.loanLimits[key]); if (Number.isFinite(value) && value >= 0 && value <= 1000000000) limits[key] = Math.round(value); });
        if (Object.keys(limits).length) program.loanLimits = limits;
      }
      if (Array.isArray(candidate.tenures)) {
        const tenures = candidate.tenures.map(function(item) { const value = Number(item && item.value); return Number.isFinite(value) && value > 0 && value <= 3650 ? { value: Math.round(value), label: Math.round(value) + ' Days' } : null; }).filter(function(item) { return item !== null; });
        if (tenures.length) program.tenures = tenures;
      }
      const fees = {};
      feeKeys.forEach(function(key) { const fee = normalizeFee(candidate.fees && candidate.fees[key]); if (fee) fees[key] = fee; });
      if (Object.keys(fees).length) program.fees = fees;
      if (candidate.collateral && typeof candidate.collateral === 'object') program.collateral = { enabled: candidate.collateral.enabled === true, required: candidate.collateral.required === true };
      if (Object.keys(program).length) programs[type] = program;
    });
    if (Object.keys(programs).length) result.loanPrograms = programs;
  }
  if (typeof input.companyName === 'string' && input.companyName.trim().length <= 100) result.companyName = input.companyName.trim();
  if (typeof input.companyWebsite === 'string' && input.companyWebsite.trim().length <= 200) result.companyWebsite = input.companyWebsite.trim();
  if (typeof input.googleScriptUrl === 'string' && input.googleScriptUrl.trim().length <= 500) result.googleScriptUrl = input.googleScriptUrl.trim();
  ['adminEmails', 'loanManagerEmails'].forEach(function(key) {
    if (Array.isArray(input[key])) {
      const values = input[key].map(function(value) { return String(value || '').trim().toLowerCase(); }).filter(function(value) { return /^[^\\s@]+@[^\\s@]+\\.[^\\s@]+$/.test(value); });
      if (values.length) result[key] = values.filter(function(value, index) { return values.indexOf(value) === index; });
    }
  });
  if (Array.isArray(input.loanManagers)) result.loanManagers = input.loanManagers;
  return result;
}
...
function handleAdminListApplications(payload) {
  ...
  if (data.length < 2) {
    return { ok: true, total: 0, offset: offset, limit: limit, applications: [] };
  }
  ...
  rows = rows.filter(function(row) {
    if (status && String(row['Application Status'] || '').toUpperCase() !== status) return false;
    if (type && String(row['Applicant Type'] || '').toUpperCase() !== type) return false;
  ...
  rows.sort(function(a, b) {
    return dateValue_(b['Date Last Updated'] || b['Date Created']) -
  ...
  const applications = page.map(function(r) {
    return {
      ...
      applicantName: r['Applicant Type'] === 'PERSONAL' ? r['Full Name'] : r['Business Name'],
...
function handleAdminUpdateStatus(payload) {
  return withScriptLock_(function() {
    const id = String((payload && payload.applicationId) || '').trim();
    const newStatus = String((payload && payload.status) || '').trim().toUpperCase();
    ...
    if ([idIdx, statusIdx, updatedIdx, submittedIdx].some(function(v) { return v === -1; })) {
...
function handleAdminListStats() {
  ...
  for (let i = 1; i < data.length; i++) {
    const status = String(valueAt_(data[i], statusIdx) || 'DRAFT').toUpperCase();
    ...
    if (status === 'DISBURSED' || status === 'REPAID') totalLoanDisbursed += principal;
...
function jsonOut_(obj) {
  return ContentService
    .createTextOutput(JSON.stringify(obj, jsonReplacer_))
...
function withScriptLock_(callback) {
  ...
  try {
    ...
    if (!acquired) {
      throw new Error('The system is busy processing another request. Please retry.');
    }
    return callback();
  } finally {
...
function stripDataUrlPrefix_(data) {
  ...
  if (value.indexOf('data:') === 0 && commaIndex !== -1) return value.substring(commaIndex + 1);
  return value;
}
...
function getApplicantDisplayName_(app) {
  if (app.applicantType === 'BUSINESS') {
    return (app.businessInfo && app.businessInfo.businessName) ||
           (app.businessRep && app.businessRep.fullName) ||
           'Business Applicant';
  }

  return (app.personalInfo && app.personalInfo.fullName) || 'Personal Applicant';
}
