...
async function start(): Promise<void> {
  try {
    ...
    app.listen(env.API_PORT, () => {
      console.log(
        `Velo API listening on port ${env.API_PORT} (database schema: ${schema})`
      );
      const { runRepaymentReminderSweep } = require("./reminders.js");
      const { runInvestmentMaturitySweep } = require("./investments.js");
