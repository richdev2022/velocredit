...
const registerSchema = z
  .object({
    ...
    phone: z.string().min(7).max(20, "Phone number is too long"),
...
router.post("/auth/register", async (req, res) => {
  ...
  res.status(201).json({ ok: true, user: safeUser, accessToken: issueToken(user) });
});
...
router.post("/auth/admin/login", async (req, res) => {
  const parsed = loginSchema.safeParse(req.body);
  if (!parsed.success || !env.ADMIN_EMAIL || (!env.ADMIN_PASSWORD && !env.ADMIN_PASSWORD_HASH)) {
  ...
  const passwordMatches = env.ADMIN_PASSWORD_HASH
    ...
    : parsed.data.password === env.ADMIN_PASSWORD;
  if (parsed.data.email.toLowerCase() !== env.ADMIN_EMAIL.toLowerCase() || !passwordMatches) {
    res.status(401).json({ ok: false, error: "Invalid admin credentials" });
    return;
  }
  const admin = {
...
router.post("/auth/otp/request", requireAuth, async (req: AuthRequest, res) => {
  ...
  res.status(201).json({ ok: true, challengeId: challenge.id, expiresAt: challenge.expiresAt, channel: parsed.data.channel });
});
...
router.post("/borrower/applications", requireAuth, requireRole("BORROWER"), (req: AuthRequest, res) => {
  ...
  const externalCreditReport = {
    ...
    reason: "External bureau report requires configured Prembly product and borrower consent.",
  };
...
router.get("/admin/kyc-cases", requireAuth, requireRole("ADMIN"), (_req, res) => {
  res.json({
    ok: true,
    cases: kycCases.map((k) => ({
      ...k,
      bvn: k.bvn ? `***-***-${k.bvn.slice(-4)}` : undefined,
      nin: k.nin ? `***-***-${k.nin.slice(-4)}` : undefined,
      user: users.find((u) => u.id === k.userId)
        ? { id: k.userId, fullName: users.find((u) => u.id === k.userId)!.fullName, email: users.find((u) => u.id === k.userId)!.email, phone: users.find((u) => u.id === k.userId)!.phone }
        : undefined,
...
router.get("/admin/loans", requireAuth, requireRole("ADMIN"), (_req, res) => {
  res.json({ ok: true, loans: loanApplications, disbursedLoans: loans });
});
