...
export function calculateCreditScore(input: CreditScoreInput): CreditScoreResult {
  ...
  if (input.bureauScore != null) { const bureauImpact = Math.max(-80, Math.min(80, Math.round((input.bureauScore - 500) * 0.2))); score += bureauImpact; factors.push({ code: "BUREAU_INPUT", label: "Credit bureau input", impact: bureauImpact, detail: "A consented bureau result was included in this calculation." }); }
