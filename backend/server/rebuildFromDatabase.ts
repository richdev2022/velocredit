  ...
  try {
    ...
    for (const row of await db.query("SELECT * FROM kyc_cases ORDER BY created_at ASC") as Row[]) {
      const kyc: KycCase = {
        ...
        livenessStatus: strNull(row, "liveness_status") ?? undefined,
        livenessManualUploaded: bool(row, "liveness_manual_uploaded", false),
        ...
        identityPhoto: strNull(row, "identity_photo"), identityPhotoUrl: strNull(row, "identity_photo"), selfieImageData: strNull(row, "selfie_image_data"),
