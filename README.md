...
| **KYC** | BVN & NIN verification via Prembly, camera selfie face comparison (Prembly face-comparison API), document uploads to Google Drive, KYC reuse of loan-application documents, smartphone QR hand-off, manual review escalation with admin email alerts, OTP-confirmed verification, admin case review per requirement, KYC gates on investing/withdrawing/applying |
...
- **Face comparison** (replaces the old embedded widget liveness check): the customer takes a **camera selfie** (no uploads) through a guided flow — instructions modal, smartphone QR hand-off (`POST /me/kyc/face-comparison/handoff` + `/handoff/claim`, short-lived stateless token, mobile page `/face-verify`), oval head guide and spoken audio directions. The selfie is compared against the BVN/NIN government portrait via Prembly `POST /verification/biometrics/face/comparison` (`POST /me/kyc/face-comparison/verify`, both images passed as base64). A confident match auto-approves the liveness checklist item; otherwise the customer can **try again** or **submit for manual review** (`POST /me/kyc/face-comparison/manual-review`) which emails the back-office team — admins then compare selfie vs portrait in the KYC review workspace and approve the Liveness requirement manually.
- **Robust verdict ladder** (a genuine customer must never be blocked by one provider opinion): the verify route tries, in order — (a) Prembly face comparison (raw base64), (b) the same endpoint with **data-URI** transport when the first call completed but did not pass, (c) Prembly **BVN/NIN-with-face** (`bvn_w_face`/`nin_w_face`) which matches the selfie against the *fresh* NIBSS/NIMC portrait fetched server-side, and (d) the **custom in-house matcher** (`backend/server/faceMatch.ts`) — face-api.js face detection + 68-point landmarks + 128-d embeddings on the pure-JS TensorFlow CPU backend (model weights vendored in `backend/server/faceapi-models/`, no native binaries). The first attempt that clears its threshold wins; every attempt is recorded on the verification event for admin review, and transport failures are reported as *unavailable* (verdict unknown) rather than "no match". A response_code `00` with an explicit `status: false` is correctly treated as **no match**.
- **Live desktop↔phone signal**: the desktop QR panel polls `GET /me/kyc/face-comparison/status` every 2.5s — the moment the phone's attempt is recorded, the desktop shows the verdict (success with confidence + the captured selfie, a failure banner with the provider message while still listening for retries, or the manual-review acknowledgement). The "I've completed verification on my phone" button also calls the same endpoint and only proceeds on an actual SUCCESS — it never assumes.
- Tuning: `PREMBLY_FACE_COMPARISON_PATH` (endpoint path), `PREMBLY_FACE_MATCH_MIN_CONFIDENCE` (minimum match confidence, default 60) and `CUSTOM_FACE_MATCH_MAX_DISTANCE` (custom matcher same-person distance threshold, default 0.55 — face-api's standard is 0.6).
