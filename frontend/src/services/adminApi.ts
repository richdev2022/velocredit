...
async function request<T>(path: string, options: RequestInit = {}): Promise<T> { const headers = new Headers(options.headers); headers.set("Content-Type", "application/json"); if (token()) headers.set("Authorization", `Bearer ${token()}`); const response = await fetch(`${API_URL}${path}`, { ...options, headers }); const body = await response.json().catch(() => ({})); if (response.status === 401) { clearAdminToken(); window.dispatchEvent(new Event("velo:admin-unauthorized")); } if (!response.ok) throw new Error(body.error || body.message || "Admin API request failed"); return body as T; }
