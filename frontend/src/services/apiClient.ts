...
export interface LoanProduct { id: string; name: string; description?: string; /** Explicit borrower-flow mapping — the admin's choice, not a name guess. */ programType?: "PERSONAL" | "BUSINESS" | "BOTH" | null; minAmountNaira: number; maxAmountNaira: number; /** Pre-selected amount on the borrower form (within [min,max]). */ defaultAmountNaira?: number; defaultTenureDays?: number; /** Allowed tenor list (days) — the borrower tenure picker renders exactly this. */ tenureDays?: number[]; /** Per-tenor MONTHLY interest rates (easimoney style) — interest for tenor T = principal × monthlyRate% × (T/30). Tenors without an entry use the base rate math. status: AVAILABLE (default) / LOCKED (visible but not selectable, rate hidden) / HOT (selectable + Hot badge). */ tenorInterestRates?: Array<{ tenorDays: number; monthlyRatePercent: number; status?: "AVAILABLE" | "LOCKED" | "HOT" }>; interestRatePercent: number; interestType: "SIMPLE_FLAT" | "REDUCING_BALANCE" | "ANNUALIZED"; processingFeePercent: number; /** One-off administration fee on the loan amount (percent). */ serviceFeePercent: number; lateFeePercent: number; lateFeeType: "ONE_TIME" | "COMPOUNDING_DAILY" | "COMPOUNDING_MONTHLY"; gracePeriodDays: number; /** Collateral section shown to applicants of this product. */ collateralEnabled?: boolean; /** When true, collateral details + media are required before submission. */ collateralRequired?: boolean; isActive: boolean; version: number; createdAt: string; updatedAt?: string; }
...
export interface AdminCreateLoanProductInput { name: string; description?: string; programType?: "PERSONAL" | "BUSINESS" | "BOTH" | null; minAmountNaira: number; maxAmountNaira: number; defaultAmountNaira?: number; defaultTenureDays?: number; tenureDays?: number[]; /** Per-tenor MONTHLY interest rates (easimoney style); entries must reference tenureDays tenors. An empty array clears the map on PATCH. */ tenorInterestRates?: Array<{ tenorDays: number; monthlyRatePercent: number; status?: "AVAILABLE" | "LOCKED" | "HOT" }> | null; interestRatePercent: number; interestType?: "SIMPLE_FLAT" | "REDUCING_BALANCE" | "ANNUALIZED"; processingFeePercent?: number; serviceFeePercent?: number; lateFeePercent?: number; lateFeeType?: "ONE_TIME" | "COMPOUNDING_DAILY" | "COMPOUNDING_MONTHLY"; gracePeriodDays?: number; collateralEnabled?: boolean; collateralRequired?: boolean; isActive?: boolean; }
