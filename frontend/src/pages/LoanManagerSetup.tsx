...
export default function LoanManagerSetup() {
  ...
  return <Layout showHomeLink={false}><div className="max-w-md mx-auto py-10"><form onSubmit={submit} className="velo-card p-6 space-y-4"><div><h1 className="text-xl font-bold text-velo-900">Set Loan Manager Password</h1><p className="text-sm text-slate-500 mt-1">Choose a password of at least 10 characters. This invitation is valid for 24 hours.</p></div><PasswordInput label="New Password" value={password} onChange={e=>setPassword(e.target.value)} autoComplete="new-password" minLength={10} required /><PasswordInput label="Confirm Password" value={confirm} onChange={e=>setConfirm(e.target.value)} autoComplete="new-password" minLength={10} required />{message&&<div className="rounded-lg bg-emerald-50 border border-emerald-100 p-3 text-sm text-emerald-700">{message}<div className="mt-2"><Link className="underline font-semibold" to="/admin">Go to admin login</Link></div></div>}{error&&<div className="rounded-lg bg-red-50 border border-red-100 p-3 text-sm text-red-700">{error}</div>}<button className="btn-primary w-full" disabled={busy||!!message}>{busy?"Saving…":"Set Password"}</button></form></div></Layout>;
}
