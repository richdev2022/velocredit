...
import { adminListDisbursements, adminPatchUserStatus, adminResetKycCategory, type KycResetCategory, type LoanDisbursement } from "../../services/adminApi";
...
export default function AdminBorrowerDetail({ borrower, onBack, onSelectLoan }: Props) {
  ...
  const [isActive, setIsActive] = useState(borrower.isActive !== false);
  ...
  async function toggleStatus() {
    const next = !isActive;
    if (!next && !confirm(`Deactivate user "${borrower.fullName}"? They will not be able to log in.`)) return;
    setActionBusy("status");
    setError("");
    try {
      await adminPatchUserStatus(borrower.id, next);
      setIsActive(next);
    } catch (err) {
      setError(err instanceof Error ? err.message : "Unable to update borrower status.");
    } finally {
      setActionBusy("");
    }
  }

  async function resetKyc(category: KycResetCategory) {
    const labels: Record<KycResetCategory, string> = { BVN: "BVN", NIN: "NIN", LIVENESS: "liveness", ADDRESS: "proof of address", ALL: "all KYC information" };
    if (!confirm(`Reset ${labels[category]} for "${borrower.fullName}"? The borrower will need to complete this verification again.`)) return;
    setActionBusy(`kyc-${category}`);
    setError("");
    try {
      await adminResetKycCategory(borrower.id, category);
    } catch (err) {
      setError(err instanceof Error ? err.message : `Unable to reset ${labels[category]}.`);
    } finally {
      setActionBusy("");
    }
  }
  ...
  return <div className="space-y-5 animate-fade-in">
    ...
    <section className="velo-card p-5 sm:p-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        ...
        <div className="flex flex-wrap gap-2 text-xs font-semibold"><span className="rounded-full bg-slate-100 px-3 py-1.5 text-slate-700 dark:bg-slate-800 dark:text-slate-300">KYC: {borrower.kycStatus || "NOT_STARTED"}</span><span className={`rounded-full px-3 py-1.5 ${!isActive ? "bg-red-50 text-red-700" : "bg-emerald-50 text-emerald-700"}`}>{!isActive ? "Inactive" : "Active"}</span></div>
      </div>
    </section>
    <section className="velo-card p-5 sm:p-6">
      <h2 className="font-semibold text-velo-900 dark:text-white">Borrower controls</h2>
      <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">Manage access and reset KYC requirements from this borrower’s record.</p>
      <div className="mt-4 flex flex-wrap gap-2">
        <button type="button" className="btn-secondary text-xs" disabled={actionBusy === "status"} onClick={() => void toggleStatus()}>{actionBusy === "status" ? "Saving…" : !isActive ? "Activate borrower" : "Deactivate borrower"}</button>
        {(["BVN", "NIN", "LIVENESS", "ADDRESS", "ALL"] as KycResetCategory[]).map((category) => <button key={category} type="button" className="btn-secondary text-xs" disabled={actionBusy !== ""} onClick={() => void resetKyc(category)}>{actionBusy === `kyc-${category}` ? "Resetting…" : category === "ALL" ? "Reset full KYC" : `Reset ${category === "ADDRESS" ? "address" : category.toLowerCase()}`}</button>)}
      </div>
    </section>
    {error && <div className="rounded-lg border border-red-100 bg-red-50 p-3 text-sm text-red-700 dark:border-red-900/40 dark:bg-red-900/20 dark:text-red-400">{error}</div>}
