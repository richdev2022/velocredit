...
export default function InvestorWithdrawalForm({ available, onSuccess }: Props) {
  ...
  return <section className="velo-card p-4 sm:p-5 lg:p-6">
    ...
    <form className="mt-5 grid gap-4 sm:grid-cols-2" onSubmit={submit}>
      <label className="block"><span className="velo-label">Amount (NGN)</span><div className="relative mt-1"><span className="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-4 text-sm font-bold text-slate-500">₦</span><input className="velo-input !pl-12" type="number" min="1" max={available} value={amount} onChange={(event) => setAmount(event.target.value)} placeholder="Enter amount" required /></div></label>
